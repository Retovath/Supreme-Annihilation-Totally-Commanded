...
public class HUD : MonoBehaviour {
	...
	//Enums are powerfull "arrays" of items of which i can assign a string which represents a particular data type, without acutally creating a new datatype
	...
	private const int ICON_WIDTH = 32, ICON_HEIGHT = 32, TEXT_WIDTH = 128, TEXT_HEIGHT = 20;
	...
	void Start () {
		...
		for(int i = 0; i < resources.Length; i++) {
			...
			case "Minerals":
				...
				resourceLimits.Add(ResourceType.Minerals, 0);
				break;
			case "Forcium":
				resourceImages.Add(ResourceType.Forcium, resources[i]);
				resourceValues.Add(ResourceType.Forcium, 0);
				resourceLimits.Add(ResourceType.Forcium, 0);
				break;
			case "Units":
				resourceImages.Add(ResourceType.UnitCap, resources[i]);
				resourceValues.Add(ResourceType.UnitCap, 0);
				resourceLimits.Add(ResourceType.UnitCap, 0);
				break;
			default: break;
			}
		}
	...
	private void DrawResourceBar() 
	{
		...
		int topPos = 0, iconLeft = 8, textLeft = 20;
		...
		iconLeft += TEXT_WIDTH;
		textLeft += TEXT_WIDTH;
		DrawResourceIcon(ResourceType.Forcium, iconLeft, textLeft, topPos);
		iconLeft += TEXT_WIDTH;
		textLeft += TEXT_WIDTH;
		DrawResourceIcon (ResourceType.UnitCap, iconLeft, textLeft, topPos);
		GUI.EndGroup();
	}
	...
	private void DrawMouseCursor()
		//does exactly what you think it does, creates a new mouse cursor on screen
		//uses system cursor untill you have something specific (gonna update that so we have something shiny at some future date
	{
		//error fixing tehe
		bool mouseOverHud = !MouseInBounds() && activeCursorState != CursorState.PanRight && activeCursorState != CursorState.PanUp;
		if (mouseOverHud){
			//!mouseInBounds checks for if my cursor is over top of 
			Cursor.visible = true;
		}
		else{
	...
	public void SetResourceValues(Dictionary<ResourceType, int> resourceValues2, Dictionary<ResourceType, int> resourceLimits2)
	{
				this.resourceValues = resourceValues2;
				this.resourceLimits = resourceLimits2;
	}
	...
	private void DrawResourceIcon(ResourceType type, int iconLeft, int textLeft, int topPos) {
		...
		GUI.Label (new Rect(textLeft-8, topPos+24, TEXT_WIDTH, TEXT_HEIGHT), text);
	}
